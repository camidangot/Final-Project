...
win = visual.Window([600,400], color='black', fullscr=0) #creating a window

#Camila - intructions and buttons
...
#making a dictionary for the key - associating languages with a color
...
#loop for buttons to create the language list - this way only languages that players pick are appended to the list
notdone = True
languages = []
while notdone:
...
languages = list(set(languages)) # will remove the extra languages

lang_text = visual.TextStim(win, text='Thanks, you picked\n '+ "\n".join(languages))
...
#instructions for an example - showing players how the image will appear on the screen and what you should type
...
#aisha's code begins: worked on randomization, making a dataframe with languages/pictures, and making a language/color dictionary
...
#couple of lines from ariana's code
#making lists/dictionaries for code at the bottom for the score and time for each language respectively
...
for trial in range(5):
	...
	#the code that aisha primarily wrote ends!

	#ariana's code begins
	...
	#making a text box and allows for them to type and including clock for reaction time
	...
	#loop showing correct or incorrect answer
	print(answer)
...
#second part of dictionaries for time and score
final_scores = {}
mean_time = {}
...
#where arian's code ends

#camila part 2
#creating a loop to create a string telling the scores
...
#creating a loop to create a string telling the mean time
...
#display the string of mean time and score to players
ready_text = visual.TextStim(win, text= intro_string, color="black", pos=(0,.5))
end_text = visual.TextStim(win, text= next_string, color="black", pos=(0,-.4))
ready_text.draw()
end_text.draw()
win.flip()
event.waitKeys(maxWait=30, keyList=['space'], clearEvents=True)
